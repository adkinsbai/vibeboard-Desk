...
- **Agent 实现模式选择**：聊天框右下角可切换“自研 Agent / Codex 硬件模式”。Codex 模式现在走独立 `codex-hardware-agent` 桥接层，被限制在 VibeBoard 480×360 硬件嵌入式 UI 设计、生成、验证和部署确认范围内，后端会返回 `mode_boundary` 与 `codex_bridge`，不开放通用电脑操作。
...
3. **资产理解**：上传资产会进入会话级 Asset Library。Agent 看到的是文件类型、大小、用途建议、文本/组件摘要、安全信号和聚合产品设计简报，而不是不受控执行上传代码；`.zip`、`.tar`、`.tgz`、`.gz` 包会被解析为保留目录结构的素材清单，危险路径、超限文件和不支持的压缩方式会被拒绝。
4. **实现模式**：用户可选择自研 Agent 或 Codex 硬件模式；两种模式都必须遵守硬件嵌入式边界。Codex 模式会通过 `codex-hardware-agent` 独立提示词和 `codex_bridge` 元数据工作，只服务硬件小屏设计、生成、验证和明确部署确认。
