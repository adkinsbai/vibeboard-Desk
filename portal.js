...
function renderBoards() {
  ...
  boardCatalog.querySelectorAll("[data-board-enter]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      if (!board) return;
      selectBoard(board);
      location.href = board.route || `/studio/workbench?board=${encodeURIComponent(board.id)}`;
    });
  });
}
...
function renderMyDevices() {
  ...
  myDeviceList.querySelectorAll("[data-device-open]").forEach(button => {
    button.addEventListener("click", () => {
      ...
      location.href = device.route || `/studio/workbench?board=${encodeURIComponent(device.board_id || "taishan-gray")}&device=${encodeURIComponent(device.serial)}`;
    });
  });
}
