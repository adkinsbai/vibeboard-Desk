...
function resolveStaticFilePath(pathname) {
  ...
  const normalizedPath = decodedPath === "/"
    ...
    : decodedPath === "/studio" || decodedPath === "/studio/" || decodedPath === "/studio/devices"
      ? "/portal.html"
      : decodedPath === "/studio/workbench" || decodedPath === "/studio/workbench/"
        ? "/index.html"
        : decodedPath === "/studio/market" || decodedPath === "/studio/market/"
          ? "/market.html"
          : decodedPath.startsWith("/studio/")
            ? `/${decodedPath.slice("/studio/".length)}`
            : decodedPath;
...
async function route(req, res) {
  ...
  try {
    ...
    if (isDatabaseUnavailable()) {
      if (req.method === "GET" && PUBLIC_DEPLOYMENT && (url.pathname === "/workbench" || url.pathname === "/studio/workbench")) {
    ...
    if (req.method === "GET" && PUBLIC_DEPLOYMENT && (url.pathname === "/workbench" || url.pathname === "/studio/workbench") && !(await currentUser(req))) {
      res.writeHead(302, { Location: "/studio", "Cache-Control": "no-store" });
      res.end();
      return;
    }
    if (req.method === "GET" && url.pathname === "/portal.html") {
      res.writeHead(302, { Location: "/studio", "Cache-Control": "no-store" });
      res.end();
      return;
    }
    if (req.method === "GET" && url.pathname === "/workbench") {
      res.writeHead(302, { Location: `/studio/workbench${url.search}`, "Cache-Control": "no-store" });
      res.end();
      return;
    }
    if (req.method === "GET" && url.pathname === "/market.html") {
      res.writeHead(302, { Location: `/studio/market${url.search}`, "Cache-Control": "no-store" });
      res.end();
      return;
    }
