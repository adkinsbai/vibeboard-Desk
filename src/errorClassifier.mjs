...
const ERROR_PROFILES = Object.freeze({
  ...
  no_api_key: {
    ...
    suggestion: "在右上角 Model 中配置 API Key，或用环境变量 VIBEBOARD_LLM_API_KEY / DEEPSEEK_API_KEY 启动服务。",
    retryable: false,
    statusCode: 400,
    nextActions: ["打开模型配置", "填写 API Key"],
  },
  missing_model_config: {
    label: "AI model is not configured",
    stage: "model_config",
    userMessage: "当前没有配置完整的 AI 模型，平台不会用预置应用冒充本次生成。",
    suggestion: "在右上角 Model 中填写 Provider、模型名称和 API Key 后再试。",
    retryable: false,
    statusCode: 400,
    nextActions: ["打开模型配置", "填写模型名称", "填写 API Key"],
  },
  llm_auth: {
