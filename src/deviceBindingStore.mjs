...
function normalizeInventoryDevice(device = {}) {
  ...
  return {
    ...
    route: String(device.route || `/studio/workbench?board=${encodeURIComponent(boardId)}&device=${encodeURIComponent(serial)}`).trim(),
  };
}
...
function publicDevice(row = {}) {
  ...
  return {
    ...
    route: row.route || `/studio/workbench?board=${encodeURIComponent(row.board_id || "taishan-gray")}&device=${encodeURIComponent(row.serial || "")}`,
